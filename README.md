# SillyTavern Incremental Save + Image Cache + Token Fast Estimate

SillyTavern 性能优化补丁，包含三个功能：

1. **增量保存** — 发新消息时只上传新增内容，不再全量上传整个聊天记录
2. **外部图片缓存** — 聊天中的外部图片（如 catbox.moe）通过服务端代理缓存，避免重复下载
3. **Token 快速估算** — 首次 token 计数使用字符估算，异步回填真实值，消除发消息前的长时间卡顿
...
SillyTavern 在发送 AI 请求前，需要逐条消息计算 token 数来决定哪些消息放进上下文窗口。当 token 缓存为空时（首次加载、切换聊天），会发起 50+ 个串行 HTTP 请求，每个 200-500ms，累计延迟 **15-20 秒**。

本补丁将阻塞式 token 计数改为**估算 + 异步回填**：

```
首次（缓存未命中）→ 立刻用字符数估算（0ms）→ 后台异步获取真实值写入缓存
再次（缓存命中）  → 直接用缓存的真实值（0ms）
```

### 核心机制

- **快速估算**：使用与服务端一致的 `字符数 / 3.35` 比率，缓存未命中时立刻返回估算值
- **异步回填**：后台发 HTTP 请求获取真实 token 数，写入客户端缓存（IndexedDB）
- **下次精确**：第二次发消息时，所有消息已有缓存的真实值，零网络请求
- **估算偏差可接受**：可能导致首次少包含 1-2 条旧消息，对回复质量几乎无影响
...
1. 清除浏览器缓存（或使用无痕模式）
2. 打开一个聊天，发送消息
3. Network 面板中 `save-append` 和 `generate` 之间应几乎无延迟（<2s）
4. `tokenizers/openai/count` 请求应出现在 `generate` 之后（异步后台执行）
5. 再发一条消息 → `tokenizers/openai/count` 请求大幅减少（缓存命中）
...
| `public/scripts/tokenizers.js` | `countTokensOpenAIAsync()` 缓存未命中时改为估算 + 异步回填 |
